...
public class Device : GLib.Object{
	...
	public static Gee.ArrayList<Device> get_disk_space_using_df(
		...
		*/
		...
		/dev/sda3      ext4        25070M  19508M     4282M  83% /mnt/indicator-diskman
		*/
	...
	public static Device? luks_unlock_using_daemon(Device dev_locked){
		...
		var list = get_block_devices_using_lsblk();
		foreach(var part in list){
			if (part.pkname == dev_locked.kname){
				dev_unlocked = part;
				break; 
			}
		}
	...
	// do not use
	public static Device? luks_unlock_udisks_from_console(Device dev_locked){

		/* There's no way to unlock without user interaction.
		 * udisksctl will prompt user for passphrase
		 * */

		/* AVOID THIS FUNCTION
		 * udisksctl will hang if device to be unlocked is present in crypttab file
		 * */

		Device dev_unlocked = null;

		var cmd = "udisksctl unlock -b %s".printf(dev_locked.device);
		log_debug(cmd);
	...
	// do not use
	public static Device? luks_unlock_udisks_from_vte_terminal(Device dev_locked){

		/* There's no way to unlock without user interaction.
		 * This method will display a small terminal window
		 * */
		...
		var temp_script = save_bash_script_temp(cmd + "\nsleep 3s\n");
	...
	public static bool luks_lock(Device dev_unlocked, out string message){

		Device dev_luks = null;
		if (dev_unlocked.is_encrypted_partition()){
			dev_luks = dev_unlocked;
		}
		else if (dev_unlocked.is_on_encrypted_partition() && (dev_unlocked.has_parent())){
			dev_luks = dev_unlocked.parent;
		}
		
		var cmd = "cryptsetup luksClose %s".printf(dev_luks.device);
		log_debug(cmd);
		...
		var dev = dev_luks.query_changes();
		
		return (dev != null) && !dev.has_children();
	}

	public static bool luks_lock_udisks(Device dev_unlocked, out string message){
		...
		var cmd = "udisksctl lock -b %s".printf(dev_luks.device);
		log_debug(cmd);
